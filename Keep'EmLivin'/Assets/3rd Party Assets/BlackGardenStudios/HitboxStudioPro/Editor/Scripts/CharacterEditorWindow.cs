...
namespace BlackGardenStudios.HitboxStudioPro
{
    public class CharacterEditorWindow : EditorWindow
    {
        ...
        bool drawDefaultInspector = false;
        ...
        {
            SavePrefab(target, uiTarget, false);
        }

        private void OnDestroy()
        {
            SavePrefab(target, uiTarget); 
        }

        private void SavePrefab(GameObject instance, GameObject prefab, bool unload = true)
        {
            if (instance != null && prefab != null && PrefabUtility.IsPartOfAnyPrefab(prefab))
            {
                PrefabUtility.SaveAsPrefabAsset(instance, AssetDatabase.GetAssetPath(prefab));
                if(unload) PrefabUtility.UnloadPrefabContents(instance);
            }
        }

        private GameObject LoadPrefab(GameObject prefab)
        {
            if (PrefabUtility.IsPartOfAnyPrefab(prefab))
                return PrefabUtility.LoadPrefabContents(AssetDatabase.GetAssetPath(prefab));
            else
                return prefab;
        }
#endif
        ...
        void OnGUI()
        {
            ...
            if (hitboxManager != null)
            {
                ...
                if (m_HitboxInspector == null)
                {
                    ...
                    m_SpriteRenderer = target.GetComponentInChildren<SpriteRenderer>();
                }

                var palette = character == null ? null : character.ActivePalette;

                if(m_SpriteRenderer != null)
                    RenderSprite(m_SpriteRenderer, palette, previewPaletteIndex);
            }
            ...
            var previoustarget = uiTarget;
            uiTarget = (GameObject)EditorGUILayout.ObjectField(uiTarget, typeof(GameObject), false);

            if(previoustarget != uiTarget)
            {
#if UNITY_2018_3
                SavePrefab(target, previoustarget);

                if (uiTarget != null)
                    target = LoadPrefab(uiTarget);
                else
#endif
                    target = uiTarget;
